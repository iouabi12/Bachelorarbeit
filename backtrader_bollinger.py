...
output_excel_file = r'C:\Users\49163\Desktop\alles\Krypto\All time High\Kapitalentwicklung_str1_BB_0,88%_1,75%.xlsx'
...
def load_resistance_data():
    ...
    tickers_df = pd.read_excel(input_file_path, sheet_name='BB', usecols=['Symbol'])
    return tickers_df
...
# Haupt-Schleife
    ...
    for ticker in tickers:
        ...
        tickers_df["EMA200"] = tickers_df["Close"].ewm(span=200, adjust=False).mean()
        lower_band = calculate_bollinger_bands(tickers_df)
        ...
        ema200_value = filtered_1hdata["EMA200"].iloc[-1].item()
        lower_band_latest = lower_band.iloc[-1].item()
        lower_band_before_latest = lower_band.iloc[-2].item()
        price_touches_lower_band1 = (vorlast_close < lower_band_before_latest) & (last_close > lower_band_latest) & (last_close < lower_band_latest * 1.006)
        # Breakout-Bedingungen überprüfen
        price_above_ema200 = last_close >= ema200_value
        body = abs(last_close - last_open)
        upper_wick = last_high - last_close
        diff_percent = (last_close - lower_band_latest) / lower_band_latest * 100
        str1_bb_ema200 = price_above_ema200 & price_touches_lower_band1
        if (
            last_close >= ema200_value
            and (vorlast_close < lower_band_before_latest) & (last_close > lower_band_latest) & (last_close < lower_band_latest * 1.006)
            and last_close > 30
            and last_volume > avg_volume_50
            #and last_open <= vorlast_close * 1.0005
            and body >= upper_wick * 2
            and last_close > last_open):
            print(f"Breakout bei {ticker}: last_ema9 bei {lower_band_latest}, Last Close bei {last_close}, Last Open bei {last_open}")
